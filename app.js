...
function initLeafletMap() {
  ...
  leafletMap = L.map('leaflet-map').setView([41.6, -93.6], 13);
...
function changeBasemap(type) {
  ...
  if (type === 'satellite') {
    tileLayer = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{attribution:'Tiles © Esri',maxZoom:19});
  } else {
    tileLayer = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap',maxZoom:19});
  }
...
function locateUser() {
  ...
  leafletMap.on('locationfound', e => {
    L.marker(e.latlng).addTo(leafletMap).bindPopup('Your location').openPopup();
  });
}
function toggleDrawMode() {
  drawMode = !drawMode;
  document.getElementById('gps-plot-info').textContent = drawMode
    ? '✏️ Click on the map to place plot markers' : '';
}
function onMapClick(e) {
  if (!drawMode || !currentDesign.builtPlots?.length) return;
  const idx = currentDesign.gpsPlots.length;
  if (idx >= currentDesign.builtPlots.length) { alert('All plots placed!'); return; }
  const plot = currentDesign.builtPlots[idx];
  const color = plot.color || plot.wpColor || '#3B6D11';
  const marker = L.circleMarker(e.latlng, {radius:10, color, fillColor:color, fillOpacity:0.8, weight:2})
    .addTo(leafletMap)
    .bindPopup(`<strong>${plot.id}</strong><br>${plot.trtName}`);
  currentDesign.gpsPlots.push({plotId:plot.id, lat:e.latlng.lat, lng:e.latlng.lng});
  mapPlots.push(marker);
  document.getElementById('gps-plot-info').textContent = `${currentDesign.gpsPlots.length}/${currentDesign.builtPlots.length} plots placed`;
}
function clearMapPlots() {
  mapPlots.forEach(m => leafletMap.removeLayer(m));
  mapPlots = [];
  currentDesign.gpsPlots = [];
  document.getElementById('gps-plot-info').textContent = '';
}
...
function exportPDF() {
  ...
  doc.text('FieldStat – Trial Analysis Report', 14, 12);
  ...
  doc.text(`Generated by FieldStat · ${new Date().toLocaleDateString()}`, 14, 285);
